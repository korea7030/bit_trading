...
error_bot_token = os.getenv('ERROR_TELEGRAM_TOKEN')
...
def buycoin_mp(target_item, buy_amount):
    try:
        ...
        ######### error key has, return blank ########
        if 'error' in rtn_data:
            #if rtn_data['error']['name'] == 'market_offline':
            return ''

        # 메세지 조립
        message = '\n\n[시장가 매수 완료!]'
        message = message + '\n\n- 대상종목:' + str(target_item)
        message = message + '\n- 매수금액:' + str(buy_amount)
        message = message + '\n- 주문시각:' + str(rtn_data['created_at'])
        send_teleram_message(message)
...
def buycoin_tg(target_item, buy_amount, buy_price):
    try:
        ...
        message = '\n\n[지정가 매수요청 완료!]'
        message = message + '\n\n- 대상종목:' + str(target_item)
        message = message + '\n- 매수요청 금액:' + str(buy_amount)
        message = message + '\n- 주문요청 시각:' + str(rtn_data['created_at'])
        send_teleram_message(message)
...
def sellcoin_mp(target_item, cancel_yn):
    try:
        ...
        print(target_item, cur_balance)

        #if Decimal(str(cur_balance)) < Decimal(str(5000)):
        #    return None
        ...
        message = '\n\n[시장가 매도 완료!]'
        message = message + '\n\n- 대상종목:' + str(target_item)
        message = message + '\n- 매도 금액:' + str(cur_balance)
        message = message + '\n- 주문 시각:' + str(rtn_data['created_at'])
        send_teleram_message(message)
    ...
    # ----------------------------------------
        ...
        logging.info("시장가 매도 : "+ str(e))
        # 잔고 부족 등의 error가 난 경우 return None
        return None
...
# -----------------------------------------------------------------------------
# 계좌 조회
def get_accounts(except_yn, market_code, want_market_code=None):
    try:
                ...
                if market_code + '-' + account_data_for['currency'] == market_item_list_for['market']:
                    ...
                    if except_yn == "Y" or except_yn == "y":
                        if account_data_for['currency'] != "KRW" and Decimal(str(account_data_for['avg_buy_price'])) * (
                                ...
                                if want_market_code:
                                    if want_market_code == market_code + '-' + account_data_for['currency']:
                                        rtn_data.append(
...
def get_ticker(target_itemlist):
    try:
        ...
        # print('upbit.py !!!!!!!!!!!!! : ', target_itemlist)
        if isinstance(target_itemlist, str):
            querystring = {"markets": target_itemlist}
        else:
            querystring = {"markets": ','.join(target_itemlist)}
...
# -----------------------------------------------------------------------------
# - Name : get_order_uuid
# - Desc : 주문 내역 조회 by UUID
# - Input
#   1) order_uuid : 주문 키
# - Output
#   1) 주문 내역 조회
# -----------------------------------------------------------------------------
def get_order_uuid(order_uuid):
    try:
        ...
        res = send_request("GET", server_url + "/v1/order", query, headers)
        rtn_data = res.json()
...
# -----------------------------------------------------------------------------
# - Name : send_line_msg
# - Desc : Telegram 메세지 전송
# - Input
#   1) message : 메세지
# - Output
#   1) response : 발송결과(200:정상)
# -----------------------------------------------------------------------------
def send_teleram_message(message, error_flag=None):
    token = error_bot_token if error_flag else telegram_token
...
def get_current_time(mili_time):
    ...
    dt = datetime.fromtimestamp(mili_time)
    return dt
